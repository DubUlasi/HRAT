...
const CONFIRM_COPY = {
  ...
  admissibility: { description: 'This decision goes back to the Complaint Registry Head for confirmation.' },
};

const SUCCESS_COPY = {
  number: 'Complaint number processed successfully!',
...
export default function DeskOfficerComplaintDetailPage() {
  ...
  const handleConfirm = () => {
    ...
    if (type === 'number') {
      ...
      submitAdmissibilityDecision(complaint.id, payload);
    }
    if (payload?.files?.length) {
      attachDocuments(complaint.id, payload.files, 'Admissibility Decision');
    }
  ...
  const renderActionButton = () => {
    if (needsNumber(complaint, officerId)) {
      return (
        <Button variant="primary" onClick={() => setFlow({ type: 'number', step: 'confirm', payload: null })}>
          Process Complaint Number
        </Button>
      );
    }
    if (needsAdmissibilityDecision(complaint, officerId)) {
      return (
        <Button variant="primary" onClick={() => setFlow({ type: 'admissibility', step: 'input', payload: null })}>
          Submit Admissibility Decision
        </Button>
      );
    }
    return null;
  ...
  return (
    <AppShell navItems={deskOfficerNavItems} user={user}>
      ...
      <SubmitAdmissibilityDecisionModal
