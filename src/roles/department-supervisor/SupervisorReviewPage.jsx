...
export default function SupervisorReviewPage() {
  ...
  const rows = complaints
    .filter((c) => needsFindingsReview(c, officerId))
  ...
  return (
    <AppShell navItems={departmentSupervisorNavItems} user={user}>
      <PageHeader title="Review Findings" subtitle="Investigation officer findings awaiting your review before they go to the Director."
        actions={<DownloadCsvButton onDownload={() => downloadComplaintsExcel(rows, 'Review Findings')} disabled={!rows.length} />}
      />
