...
export default function DirectorNewAssignmentsPage() {
  ...
  const rows = complaints
    .filter((c) => needsDeptReview(c, departmentId) || needsAssignment(c, departmentId))
  ...
  return (
    <AppShell navItems={departmentDirectorNavItems} user={user}>
      <PageHeader
        ...
        subtitle="Complaints newly assigned to your department, accept or reject, then assign a supervisor or investigation officer."
        actions={<DownloadCsvButton onDownload={() => downloadComplaintsExcel(rows, 'New Assignments')} disabled={!rows.length} />}
      />
