...
const CONFIRM_COPY = {
  ...
  assignInvestigator: { description: 'You are assigning this complaint directly to an investigator.' },
    ...
    return (
      <AppShell navItems={departmentDirectorNavItems} user={user}>
        <div className="detail-top-nav-bar">
          <Link to="/department-director/new" className="back-to-cases-btn">
            <ArrowLeft size={16} /> Back to New Assignments
          </Link>
        </div>
    ...
    if (needsAssignment(complaint, departmentId)) {
      return (
        <>
          ...
          <Button variant="secondary" onClick={() => startFlow('assignInvestigator')}>Assign Investigator Directly</Button>
        </>
      );
    }
